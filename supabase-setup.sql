...
DROP POLICY IF EXISTS "Allow anonymous read" ON devtrack_sync_records;
...
CREATE POLICY "Allow anonymous read" ON devtrack_sync_records FOR SELECT USING (true);
...
-- 5. 启用实时订阅（可选，用于多设备实时同步）
ALTER PUBLICATION supabase_realtime ADD TABLE devtrack_sync_records;
