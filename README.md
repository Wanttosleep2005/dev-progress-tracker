...
[![Version](https://img.shields.io/badge/version-1.1-green)](CHANGELOG.md)
...
- 架构图：使用 React Flow 可视化项目文件/文件夹结构，支持拖拽、关联任务/里程碑、AI 生成建议和 Supabase 实时同步。
- 同步控制台：Owner 可查看实时同步日志、待同步队列、Realtime 连接状态和诊断导出。
- AI 指令增强：更丰富的上下文（里程碑、依赖、计划时间）、更严格的字段校验和质量要求。
- 任务依赖图布局优化：基于重心法减少边交叉，节点排列更清晰。
...
- 架构图：React Flow 可视化项目结构，支持拖拽、关联任务/里程碑和 AI 生成。
...
v1.1 builds on v1.0 with:

- Architecture Diagram: React Flow visualization of project file/folder structure
  with drag-and-drop, task/milestone linking, AI suggestions, and realtime sync.
- Sync Console: Owner-only live sync log viewer with queue inspection and
  diagnostics export.
- AI Command enhancements: richer context (milestones, dependencies, planned
  dates), stricter validation, and quality requirements.
- Task dependency graph layout improvements using barycenter heuristics.
...
- Architecture diagram powered by React Flow with drag-and-drop and AI generation.
